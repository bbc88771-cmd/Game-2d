...
(() => {
  ...
  // Категории, на которые «Некий» отвечает по существу ВСЕГДА, даже если
  // вопрос уже звучал, — иначе анти-повтор «съедал» ответ на «где я», «кто ты» и т.п.
  const ALWAYS_ANSWER = new Set(["who", "where", "question", "help", "emotion", "story"]);
  function nekoBrain(text) {
    ...
    if (cat !== "empty" && !ALWAYS_ANSWER.has(cat)) {
