...
(() => {
  ...
  // ВАЖНО: JS \b не понимает кириллицу, поэтому границы слова делаем через
  // lookbehind (?<![а-яё]). Иначе «еб[…]» ловит «тебе»/«небо»/«требует», а
  // «манда» — «команда»: «Некий» отвечал «Матершинник» и штрафовал доверие.
  const SWEAR_RE = /(?:бл[яе][дт]?|сук[аиоуые]|ху[йёеяи]|пизд|пид[оа]р|(?<![а-яё])[её]б[аоуёные]|объеб|уеб[аон]|муда[кч]|г[оа]ндон|говн|залуп|(?<![а-яё])манда|шлюх|мраз|долбо[её]б|нах[уй]|похуй|зае[бо])/i;
  const SWEAR_LINES = [
    "Матершинник.",
  ...
  async function nekoConverse(maxTurns = 12) {
    const DONE_RE = /^(дальше|продолж|поехали|ид[её]м|пошли|погнали|впер[её]д|готов|хватит|начн[её]м|начинаем|играть|давай начн|давай дальше|ладно дальше|го(?![а-яё])|поехали)/i;
  ...
  function sayUnique(pool) {
    ...
    recentSaid.push(line); if (recentSaid.length > 60) recentSaid.shift();
  ...
  const TOPIC_RULES = [
    ...
    ["combat",    /(как (мне )?(драться|сражаться|воевать|убива|бить|боротьс|победить)|оружие|меч[ауео]?(?![а-яё])|сражени|(?<![а-яё])бой(?![а-яё])|драк|атак|защищат|чем (бить|драть))/],
  ...
  // ЛЕКСИКОН МИРА: «Некий» отвечает ПО СУЩЕСТВУ почти на любое слово об этом
  // мире, оставаясь строго в его рамках. Проверяется для общих вопросов после
  // конкретных тем. Порядок важен — более узкие совпадения выше.
  const WORLD_LEXICON = [
    { re: /(маги[яюине]|колдов|чар[ыоуе]|заклинан|волшеб)/, lines: [
    ...
    { re: /(золот|деньг|монет|богатств|серебр)/, lines: [
    ...
    { re: /(еда|еды|жрать|есть хочу|проголод|голод|кушать|пища|пропитан)/, lines: [
    ...
    { re: /(ночь|тьма|темн|мрак|потёмк)/, lines: [
    ...
    { re: /(солнц|рассвет|закат|дн[её]м|день)/, lines: [
    ...
    { re: /(луна|звёзд|звезд|созвезд)/, lines: [
    ...
    { re: /(неб[оаеу]|небес|облак|тучи)/, lines: [
    ...
    { re: /(огон|лава|пламя|жар|пожар|горит|костёр|костер)/, lines: [
    ...
    { re: /(дет[ией]|ребён|ребен|младен|малыш)/, lines: [
    ...
    { re: /(любов|сердц|люблю|влюб)/, lines: [
    ...
    { re: /(семь|\bмать\b|мама|\bотец\b|папа|родител|\bбрат|сестр)/, lines: [
    ...
    { re: /(надежд|верю что|во что верить)/, lines: [
    ...
    { re: /(бог(?![ат])|молит|церков|храм|вера в)/, lines: [
    ...
    { re: /(сон(?![а-яё])|спать|поспать|мечт|снит|уснуть)/, lines: [
    ...
    { re: /(дорог|путь|тропа|куда (идти|мне идти|двигат)|направлен)/, lines: [
  ...
  function nekoWorldRedirect(text) {
    ...
    return sayUnique([
      ...
      `Ты тащишь сюда ${w ? "«" + w + "»" : "вопросы"} из прежней жизни. Её нет. Есть осколки, пустота и я.`,
      `${q} здесь не существует. Зато существуют твари на дальних островах и жажда. Спроси про них — отвечу.`,
      `Я говорю только об этом мире, ${n}. ${w ? "Про «" + w + "» — мимо. " : ""}Спроси про острова, воду, тех, кто выжил.`,
  ...
  function nekoBrain(text) {
    ...
    // Конкретная тема/эмоция/категория — отвечаем из её пула (это по делу).
    if (cat !== "question" && cat !== "statement" && cat !== "nonsense") {
      const node = BRAIN[cat] || BRAIN.statement;
      return sayUnique(R(node.s, node.h || node.s));
    }
    // Общий вопрос/утверждение/набор символов: сначала пытаемся ответить
    // ПО СУЩЕСТВУ в рамках мира (лексикон по ключевому слову)…
    const lex = nekoLexicon(text);
    if (lex) return lex;
    // …иначе держим разговор СТРОГО в рамках игры: внутримировой редирект,
    // который называет слово игрока и возвращает его к этому миру.
    if (cat === "nonsense") return sayUnique(R(BRAIN.nonsense.s, BRAIN.nonsense.h));
    if (cat === "statement") return sayUnique(R(BRAIN.statement.s, BRAIN.statement.h));
    return nekoWorldRedirect(text);
  }
